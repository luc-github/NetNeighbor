...
TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "Override or add connection endpoints per protocol. For 'custom' scheme, enter the command template in IP/Host ({ip}, {port}, {name}, {type}, {category}, {url}). IP/Host and Port are optional for other schemes.": {
        "fr": "Remplacez ou ajoutez des points de connexion par protocole. Pour le schème 'custom', entrez le modèle de commande dans IP/Hôte ({ip}, {port}, {name}, {type}, {category}, {url}). IP/Hôte et Port sont optionnels pour les autres schèmes.",
        "de": "Verbindungsendpunkte pro Protokoll überschreiben oder hinzufügen. Für das Schema 'custom' geben Sie die Befehlsvorlage in IP/Host ({ip}, {port}, {name}, {type}, {category}, {url}) ein. IP/Host und Port sind für andere Schemata optional.",
        "es": "Anular o agregar puntos de conexión por protocolo. Para el esquema 'custom', ingrese la plantilla de comando en IP/Host ({ip}, {port}, {name}, {type}, {category}, {url}). IP/Host y Puerto son opcionales para otros esquemas.",
        "it": "Sovrascrivere o aggiungere endpoint di connessione per protocollo. Per lo schema 'custom', inserire il modello di comando in IP/Host ({ip}, {port}, {name}, {type}, {category}, {url}). IP/Host e Porta sono opzionali per altri schemi.",
        "nl": "Verbindingseindpunten per protocol overschrijven of toevoegen. Voor het schema 'custom' voert u de opdrachtsjabloon in IP/Host ({ip}, {port}, {name}, {type}, {category}, {url}) in. IP/Host en poort zijn optioneel voor andere schema's.",
        "ja": "プロトコルごとに接続エンドポイントを上書きまたは追加します。'custom' スキームの場合、IP/ホスト ({ip}, {port}, {name}, {type}, {category}, {url}) のコマンドテンプレートを入力してください。IP/ホストとポートは他のスキームでは省略可能です。",
        "zh_CN": "按协议覆盖或添加连接端点。对于 'custom' 方案，请在 IP/主机 ({ip}, {port}, {name}, {type}, {category}, {url}) 中输入命令模板。对于其他方案，IP/主机和端口是可选的。",
        "zh_TW": "依協議覆蓋或新增連線端點。對於 'custom' 方案，請在 IP/主機 ({ip}, {port}, {name}, {type}, {category}, {url}) 中輸入命令範本。對於其他方案，IP/主機和連接埠是選擇性的。",
    },
    "Preferences…": {
        "fr": "Préférences…",
        "de": "Einstellungen…",
